...
static pixMap* pixMap_init(unsigned char arrayType){
	...
	pixMap* pixelMap = malloc(sizeof(pixMap));
	
	pixelMap->arrayType = arrayType;
	pixelMap->image = 0;
	pixelMap->imageWidth = 0;
	pixelMap->imageHeight = 0;
	pixelMap->pixArray_arrays = 0;
	pixelMap->pixArray_blocks = 0;
	pixelMap->pixArray_overlay = 0;
...
pixMap *pixMap_read(char *filename,unsigned char arrayType){
 ...
 //allocate the 2-D rgba arrays
  ...
  int numRows = p->imageHeight;
  int numCols = p->imageWidth;
  p->pixArray_arrays = malloc(numRows * sizeof(rgba[MAXWIDTH]));
  int count = 0;
  for (int i = 0; i < numRows; i++) {
	  for (int j = 0; j < numCols; j++) {
		  p->pixArray_arrays[i][j].r = p->image[count++];
		  p->pixArray_arrays[i][j].g = p->image[count++];
		  p->pixArray_arrays[i][j].b = p->image[count++];
		  p->pixArray_arrays[i][j].a = p->image[count++];
	  }
  }
		...
		int numRows = p->imageHeight;
		int numCols = p->imageWidth;
		p->pixArray_blocks = malloc(numRows * sizeof(rgba*));
		int count = 0;
		
		for (int i = 0; i < numRows; i++) {
			p->pixArray_blocks[i] = malloc(numCols * sizeof(rgba));
		}
		
		for (int i = 0; i < numRows; i++) {
			for (int j = 0; j < numCols; j++) {
				p->pixArray_blocks[i][j].r = p->image[count++];
				p->pixArray_blocks[i][j].g = p->image[count++];
				p->pixArray_blocks[i][j].b = p->image[count++];
				p->pixArray_blocks[i][j].a = p->image[count++];
			}
		}
 }
	else if (arrayType ==2){
  ...
  //each subsequent pointer is the previous pointer + p->imageWidth
		int numRows = p->imageHeight;
		int numCols = p->imageWidth;
		p->pixArray_overlay = malloc(numRows * numCols * sizeof(rgba*));
...
int pixMap_write(pixMap *p,char *filename){
	...
	//for arrayType 0 and arrayType 1 you have to write out a copy of the rows  to the image using memcpy
	 if (p->arrayType ==0){
		...
		for (int i = 0; i < numRows; i++) {
			for (int j = 0; j < numCols; j++) {
				p->image[count++] = p->pixArray_arrays[i][j].r;
				p->image[count++] = p->pixArray_arrays[i][j].g;
				p->image[count++] = p->pixArray_arrays[i][j].b;
				p->image[count++] = p->pixArray_arrays[i][j].a;
			}
		}
   ...
   //have to copy each row of the array into the corresponding row of the image		
		...
		for (int i = 0; i < numRows; i++) {
			for (int j = 0; j < numCols; j++) {
				p->image[count++] = p->pixArray_blocks[i][j].r;
				p->image[count++] = p->pixArray_blocks[i][j].g;
				p->image[count++] = p->pixArray_blocks[i][j].b;
				p->image[count++] = p->pixArray_blocks[i][j].a;
			}
		}
	 }
...
pixMap *pixMap_copy(pixMap *p){
	...
	if (new->arrayType ==0){
	 ...
	 memcpy(new->image, p->image, new->imageHeight * new->imageWidth * sizeof(rgba));
